...
def getData():
     ...
     df = pd.read_csv(input('Enter Something: ')) #<-- placeholder, can remove once defined.
...
def getColumnNames():
     ...
     latName, lonName = input('enter'),input('Enter2')
...
def getLocale():
     ...
     """
     lat,lon =float(input('Enter a float')),float(input('Enter a float'))
...
def computeDist(x1,y1,x2,y2):
     ...
     """
     d = float((x1-x2)**2 + (y1-y2)**2)   #<-- placeholder, can remove once defined.
